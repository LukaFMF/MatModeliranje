...
obesisceL = [1;5];
obesisceD = [4;4];
%zvVeriznica(obesisceL,obesisceD,L,eps);
...
%zvVeriznica(obesisceL,obesisceD,L,eps);
%format long;
%ordNajnizja2_5 = fzero(@(y) najnizjaAbscisa(y) - 2.5,3)
%format short;

%function najnizjiX = najnizjaAbscisa(visina)
%	L = 15;
%	obesisceL = [0;5];
%	obesisceD = [5;visina];
%	T_min = zvVeriznica(obesisceL,obesisceD,L,eps);
%	najnizjiX = T_min(1);
%end
